...
func main() {
	...
	mux.Handle("GET /config", tracing.WrapHandlerFunc(enableCORS(handleConfig), "GET /config"))

	// User endpoints (gRPC → HTTP translation)
	mux.Handle("POST /v1/users/register", tracing.WrapHandlerFunc(enableCORS(handleRegister), "POST /v1/users/register"))
	mux.HandleFunc("OPTIONS /v1/users/register", enableCORS(handleOptions))
	mux.Handle("POST /v1/auth/token", tracing.WrapHandlerFunc(enableCORS(handleLogin), "POST /v1/auth/token"))
	mux.HandleFunc("OPTIONS /v1/auth/token", enableCORS(handleOptions))
	mux.Handle("POST /v1/auth/refresh", tracing.WrapHandlerFunc(enableCORS(handleRefresh), "POST /v1/auth/refresh"))
	mux.HandleFunc("OPTIONS /v1/auth/refresh", enableCORS(handleOptions))
	mux.Handle("GET /v1/users/activate/{token}", tracing.WrapHandlerFunc(handleActivate, "GET /v1/users/activate/{token}"))
	mux.Handle("GET /v1/users/me", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleGetMe)), "GET /v1/users/me"))
	mux.Handle("PATCH /v1/users/profile", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleUpdateProfile)), "PATCH /v1/users/profile"))
	mux.HandleFunc("OPTIONS /v1/users/profile", enableCORS(handleOptions))
	mux.Handle("POST /v1/auth/logout", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleLogout)), "POST /v1/auth/logout"))
	...
	mux.Handle("GET /trips/history", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleTripHistory)), "GET /trips/history"))
	mux.Handle("GET /trips/driver-history", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleDriverHistory)), "GET /trips/driver-history"))
	mux.Handle("GET /trips/search", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleTripSearch)), "GET /trips/search"))
	mux.Handle("GET /drivers/nearby", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleNearbyDrivers)), "GET /drivers/nearby"))
	...
	mux.Handle("GET /ws/ticket", tracing.WrapHandlerFunc(enableCORS(requireAuth(handleWsTicket)), "GET /ws/ticket"))
